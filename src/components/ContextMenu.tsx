...
const ContextMenu: React.FC<ContextMenuProps> = ({ x, y, nodeId, edgeId, getData, setData, onSearchExtended, onClose, onEditAttributes }) => {
  const [_nodeDetails, setNodeDetails] = useState<any>(null);
  const [_edgeDetails, setEdgeDetails] = useState<any>(null);

  const { findNodeDetails, findEdgeDetails } = useGraphFunctions(setData, getData);
  ...
  useEffect(() => {
    if (nodeId) {
      ...
      const details = findEdgeDetails(edgeId);
      setEdgeDetails(details);
    }
  }, [nodeId, edgeId, findNodeDetails, findEdgeDetails]);
