...
const buscarContactosPorTelefono = async ({ entidad, payload }: { entidad: string; payload: Payload }) => {
    ...
    try {
      const response = await fetch('http://172.18.110.90:8087/api/search/contactosTelefono', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
const searchInspeccionVehiculo = async ({ entidad, payload }: { entidad: string; payload: Payload }) => {
  ...
  try {
    const response = await fetch('http://172.18.110.90:8087/api/search/inspeccion-vehiculo', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
